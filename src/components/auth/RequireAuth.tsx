...
import { hasActiveWarehouseAccess } from '../../lib/roles'

export function RequireAuth({ children }: { children: ReactNode }) {
  const { session, profile, warehouseAssignments, loading, signOut } = useAuth()
  ...
  if (profile.role === 'warehouse_operations' && !hasActiveWarehouseAccess(warehouseAssignments)) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="max-w-sm w-full bg-white border border-gray-200 rounded-xl p-6 text-center">
          <Clock size={24} className="mx-auto text-amber-500 mb-3" />
          <p className="text-sm font-medium mb-1">Warehouse access not assigned</p>
          <p className="text-xs text-gray-500 mb-4">
            Your account is active, but an admin still needs to assign a warehouse and responsibility.
          </p>
          <button onClick={signOut} className="text-xs text-gray-400 hover:text-gray-600">Sign out</button>
        </div>
      </div>
