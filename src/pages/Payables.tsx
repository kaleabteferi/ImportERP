...
function RecordPurchasePaymentForm({ payable, onDone, onCancel }: {
  payable: Payable
  onDone: () => void
  onCancel: () => void
}) {
  ...
  async function submit() {
    ...
    setError(null)
    try {
      await recordPurchasePayment(payable.id, amt, payable.currency as 'USD' | 'ETB', method, { sensitive, notes })
      onDone()
    } catch (e: any) {
...
function MarkExpensePaidForm({ payable, onDone, onCancel }: {
  payable: Payable
  onDone: () => void
  onCancel: () => void
}) {
  ...
  async function submit() {
    ...
    try {
      const { error } = await supabase
        ...
        .update({ is_paid: true, payment_method: method, sensitive_flag: sensitive })
        .eq('id', rawId)
