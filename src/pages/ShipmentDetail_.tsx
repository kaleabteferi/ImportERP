...
const CAT_SUGGESTIONS: Record<string, string[]> = {
  ...
  TRUCKING:         ['Truck fee – Addis route', 'Fuel', 'Driver costs', 'Security'],
...
export function ShipmentDetail() {
  ...
  const [shRes, rawItemsRes, expRes, prodRes, fxRes] = await Promise.all([
    supabase.from('shipments')
      .select('id, shipment_number, container_number, status, allocation_method, eta_djibouti, arrived_addis_date, vessel_name, bl_number, notes, supplier_id, port_of_loading, incoterm, payment_terms, truck_plate, seal_number, driver_name, suppliers(name, contact_person, email)')
      .eq('id', id)
      .single(),

    // Fetch items WITHOUT join — avoids the FK error entirely
    supabase.from('shipment_items')
      .select('id, product_id, quantity, unit_price_usd, weight_kg_total, volume_m3_total, allocated_cost_etb, unit_landed_cost_etb, cost_status, hs_code, country_of_origin, carton_qty, units_per_carton, gross_weight_per_ctn, carton_number_from, carton_number_to')
      .eq('shipment_id', id)
      .order('created_at'),

    supabase.from('shipment_expenses')
      .select('id, category, description, amount, currency, amount_etb, exchange_rate, cost_status, vendor_name, expense_date, receipt_ref')
      .eq('shipment_id', id)
      .order('expense_date'),

    // Fetch ALL products once — we'll join in JS
    supabase.from('products')
      .select('id, name, sku, unit_of_measure, weight_kg, volume_m3')
      .eq('is_active', true)
      .order('name'),
  ...
  async function saveItem() {
    ...
    const { error: err } = await supabase.from('shipment_items').insert({
      ...
      weight_kg_total: itemForm.weight_kg_total
        ...
        : prod?.weight_kg ? prod.weight_kg * qty : null,
      volume_m3_total: itemForm.volume_m3_total
        ? parseFloat(itemForm.volume_m3_total)
        : prod?.volume_m3 ? prod.volume_m3 * qty : null,
      cost_status: 'PROVISIONAL',
    })

    if (err) {
      // If columns missing, try without optional columns
      const { error: err2 } = await supabase.from('shipment_items').insert({
        shipment_id:    id,
        product_id:     itemForm.product_id,
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-gray-50/50">
                          ...
                          <td className="px-3 py-3 text-right font-mono">
                            {N(item.quantity)}{' '}
                            <span className="text-gray-400">{prod?.unit_of_measure ?? 'PCS'}</span>
                          </td>
        ...
        <div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden mb-4">
            ...
            <div className="px-6 py-3 border-t border-gray-100 text-xs text-gray-400">
              Total cartons: {N(items.reduce((s, i) => s + Math.ceil(i.quantity / 2), 0))} ·
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <InfoTip text="Price per unit in USD as shown on the PI. This is also the customs valuation basis (FOB)." />
                  </div>
                  <input
          ...
          <div className="bg-white rounded-2xl w-full max-w-md max-h-[90vh]
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {editExpId
                    ? 'Update the amount or details'
                    : 'Record a cost attached to this shipment'}
                </p>
              </div>
              <button
                onClick={() => { setExpOpen(false); setEditExpId(null) }}
                className="text-gray-400 hover:text-gray-600 transition-colors"
              >
                <X size={18} />
              </button>
            ...
            <div className="px-5 py-4 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {Object.entries(CAT_LABELS).map(([k, v]) => (
                    <button
                      key={k}
                      type="button"
                      onClick={() => { setF('category', k); setF('description', '') }}
              ...
              <div>
                ...
                <datalist id="exp-suggestions">
                  {(CAT_SUGGESTIONS[expForm.category] ?? []).map(s => (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <div className="flex items-center gap-1 mb-1">
                    ...
                    <InfoTip text="Choose the currency the invoice is in. The system automatically converts to ETB using today's customs rate. If using USD, the rate is fetched from your Forex Rates table." />
                  </div>
                  <select
                    className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg
                               focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white"
              ...
              {/* Live ETB preview */}
              {expForm.amount && expForm.currency !== 'ETB' && (
                <div className="flex items-center justify-between px-3 py-2
                                bg-amber-50 rounded-lg">
                  <span className="text-xs text-amber-700">
                    Converts to (at {fxRate} ETB/USD)
                  </span>
                  <span className="text-sm font-medium font-mono text-amber-700">
                    {N(
                      expForm.currency === 'USD'
                        ? parseFloat(expForm.amount) * fxRate
                        : parseFloat(expForm.amount) * (fxRate / 7.2)
                    )} ETB
                  </span>
                </div>
              )}
              ...
              <div>
                <div className="flex items-center gap-1 mb-1">
                  <label className="text-xs text-gray-500">Receipt / invoice ref</label>
                  <InfoTip text="The invoice number from your vendor (e.g. ERCA receipt number, freight invoice number). Used for auditing and document tracking." />
                </div>
                <input
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg
                             focus:outline-none focus:ring-2 focus:ring-blue-400 font-mono"
                  value={expForm.receipt_ref}
                  onChange={e => setF('receipt_ref', e.target.value)}
            ...
            <div className="flex items-center justify-end gap-2 px-5 py-4
              ...
              >
                {saving
                  ? <><Loader2 size={12} className="animate-spin" /> Saving…</>
                  : <><Check size={12} /> {editExpId ? 'Save changes' : 'Add expense'}</>
                }
              </button>
            </div>
          </div>
