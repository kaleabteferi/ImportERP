...
import { fetchCreditAccounts, fetchCustomersForCredit, openCreditAccount, recordCreditTransaction } from '../api/credit'
...
function RepaymentForm({ account, onDone, onCancel }: {
  ...
  async function submit() {
    ...
    setError(null)
    try {
      await recordCreditTransaction(account.id, 'repayment', amt, { method, sensitive, notes })
      onDone()
    } catch (e: any) {
          ...
          {accounts.map((a, i) => (
            <div key={a.id} className={i < accounts.length - 1 ? 'border-b border-gray-50' : ''}>
              <div className="flex items-center gap-3 px-4 py-3">
                ...
                <div className="font-mono font-medium w-24 text-right text-sm">{N(a.balance)} ETB</div>
                <button
